...
const GATE_REQUIREMENTS: Record<Phase, Array<{ key: string; message: string; isMet: (cycle: Cycle) => boolean }>> = {
  ...
  F1: [
    ...
    { key: "bmapCause", message: "La causa debe ser Motivation, Ability o Prompt.", isMet: hasBmapCause },
  ],
  F2: [
...
// BUG-4 FIX: Original code checked for literal "B=MAP" string but cycleService.ts
// uses BMapCause enum values: "Motivation" | "Ability" | "Prompt" | "Unknown".
// The gate requires a concrete cause diagnosis (not "Unknown").
function hasBmapCause(cycle: Cycle): boolean {
  const cause = cycle.cause ?? cycle.f1?.cause ?? cycle.diagnosis?.cause ?? cycle.bmapCause;
  return ["Motivation", "Ability", "Prompt"].includes(String(cause ?? ""));
}
